            ...
            <main className="m-3">
                ...
                <div className="rounded-xl grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 gap-4 p-4">
                    {categoriesList.map(category => (
                        <Link title={`Link to ${category.categoryName}`} aria-label={`Link to ${category.categoryName}`} href={`/categories/${category._id}`} key={category._id} className="bg-[#D9D9D9] hover:bg-gray-100 text-black border rounded-xl overflow-hidden shadow-md">
                            <img
