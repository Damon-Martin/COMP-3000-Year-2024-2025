...
export default function OrderButtonDesktop({ index, transactionID="TRANSACTION ID", totalPrice="XX.XX" }) {
    ...
    return (
        <Link href={`/transaction?transactionID=${transactionID}`} key={index} title={ariaLabel} aria-label={ariaLabel} className="bg-[#D9D9D9] hover:bg-gray-100 text-black mt-3 min-w-[70vw] min-h-[20vh] max-h-[20vh] rounded-lg flex flex-col">
            <div className='m-3'>
